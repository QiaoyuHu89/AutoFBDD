    ...
    if len(argList) > 12:
        ...
        if len(argList) == 12:
            if (argList[4] == '-p') and (argList[6] == '-m') and (argList[8] == '-c') and (argList[10] == '-t'):
                ...
                if (tempCoreNum >= 1) and (tempCoreNum <= 100):
...
def GetInputList(inputFolderPath, outputFolderPath_log):
    ...
    try:
        ...
        try:
            ...
            files = glob.glob(inputFolderPath + '*.mol2')
            #files = glob.glob(inputFolderPath + '*.sdf')
            for file in files:
                fileNameList.append(file)
...
def Chop(outputPathList, pool):
    ...
    try:
        ...
        for i in inputList:
            # print(i)
            if Chem.MolFromMol2File(i, sanitize=False) != None:
                try:
            #if Chem.SDMolSupplier(i)[0] != None:
                    partial_Chop(i)
                except:
                    continue
