...
from align_utils import align_mol_to_frags_elaboration, align_mol_to_frags_elaboration_edit
...
def preprocess_edit(raw_data, dataset, name, output_dir='', test=False):
    print('Parsing smiles as graphs.')
    processed_data =[]
    total = len(raw_data)
    for i, (smi_mol, smi_frags, smi_elab, abs_dist, idx) in enumerate([(mol['smi_mol'], mol['smi_frags'], 
        ...
        if test:
            smi_mol = smi_frags
            smi_elab = ''
        (mol_out, mol_in), nodes_to_keep, exit_points = align_mol_to_frags_elaboration_edit(smi_mol, smi_frags)
        if mol_out == []:
            continue
        nodes_in, edges_in = to_graph_mol(mol_in, dataset)
        nodes_out, edges_out = to_graph_mol(mol_out, dataset)
