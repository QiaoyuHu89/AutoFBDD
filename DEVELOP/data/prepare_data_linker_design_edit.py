...
pharms = ['Donor', 'Acceptor', 'NegIonizable','PosIonizable','Aromatic', 'Hydrophobe', 'LumpedHydrophobe']
def get_pharm_dict(smi):
    feats = factory.GetFeaturesForMol(Chem.MolFromSmiles(smi))
    dic = {'Donor': 0, 'Acceptor': 0, 'NegIonizable': 0,'PosIonizable': 0,'Aromatic': 0, 'Hydrophobe': 0, 'LumpedHydrophobe':0}
    for feat in feats:
        if feat.GetFamily() in pharms:
            dic[feat.GetFamily()] +=1
    return [dic['Donor'], dic['Acceptor'], dic['NegIonizable'], dic['PosIonizable'], dic['Aromatic'], dic['Hydrophobe'], dic['LumpedHydrophobe']]
...
def read_file_edit(file_path, pharm_count, add_idx=False, calc_pharm_counts=False):
    with open(file_path, 'r') as f:
        lines = f.readlines()
    num_lines = len(lines)
    data = []
