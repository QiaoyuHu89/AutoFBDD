...
def get_data_dict(bricks_list, data_path, iter_idx):
    with open(bricks_list, 'r') as fr:
        lines = fr.readlines()
    ...
    for types in types_list:
        ...
        for line in lines[1:]:
            ...
            if iter_idx == 1:
                bricks_name = os.path.splitext(newline[0])[0].split('_')[2] + '_' + os.path.splitext(newline[0])[0].split('_')[4]
            else:
                bricks_name = os.path.splitext(newline[0])[0]
            if types_name == bricks_name:
                dist = newline[-1]
                data_dict[types_name] = dist
    return data_dict

def growing_bricks(json_file, types_file, name, min, max, iter_idx):
    ...
    args['--restore'] = './pretrained_DEVELOP_model.pickle'
...
def main(bricks_list, data_path, iter_idx):
    data_dict = get_data_dict(bricks_list, data_path, iter_idx)
    for key, val in data_dict.items():
        ...
        if lines[0] != str([]):
            if round(float(val)) < 2:
                ...
                max = str(round(float(val))+2)
            growing_bricks(json_file, types_file, key, min, max, iter_idx)

if __name__=="__main__":
    # Get the arguments
    # Usage: python brick_growing.py -l app_bricks.list -d data_path -i iter_idx
    ...
    start = time.time()
    main(bricks_list, data_path, iter_idx)
    end = time.time()
    print('='*100)
    print('Running time: %s Seconds' % (end-start))
    print('brick growing done!')
