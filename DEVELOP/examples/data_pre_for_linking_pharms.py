...
def cal_dis_ang_folder(app_bricks, brick_folder, name):
    ...
    for line in lines[1:]:
        ...
        data_path = cal_dis_ang(brick1, brick2, atom_idx_1, atom_idx_2, name)
...
def obtain_linker_pharma(mol2_file, frag_sdf, output_sdf):
    ...
    pharms = ['Donor', 'Acceptor', 'NegIonizable', 'PosIonizable', 'Aromatic', 'LumpedHydrophobe']
    #pharms = ['Donor', 'Acceptor', 'Aromatic', 'PosIonizable', 'NegIonizable', 'LumpedHydrophobe']
    ...
    symbol = ['O', 'N', 'C', 'P', 'S', 'F'] # pharmacophore symbol on receptor
    antisymbol = ['N', 'O', 'P', 'C', 'S', 'F'] # pharmacophore symbol on linker
    fake_lig = '*.*'
    ...
    for sc_file in sc_bb_file[:-1]:
        ...
        if res == 'TRP':
            ...
            for sc_ent in sc_copy_feats:
                ...
                elif sc_ent[0] == 'LumpedHydrophobe':
                    sc_new_feats.remove(sc_ent)
        ...
        elif res == 'SER' or res == 'THR' or res == 'TYR' or res == 'PHE':
            sc_copy_feats = copy.deepcopy(sc_new_feats)
            for sc_ent in sc_copy_feats:
                if sc_ent[0] == 'Acceptor':
                    sc_new_feats.remove(sc_ent)
                elif sc_ent[0] == 'LumpedHydrophobe':
                    sc_new_feats.remove(sc_ent)
            sc_pharm_coords, sc_ent = obtain_sc_pharm_ent(sc_new_feats, sc_conf, dummy_pos_list, anchor_1, anchor_2)
            ...
            for sc_ent in sc_copy_feats:
                if sc_ent[0] == 'Donor':
                    for i in list(sc_ent[1]):
                        donor_idxs.append(i)
                    sc_new_feats.remove(sc_ent)
                elif sc_ent[0] == 'PosIonizable':
                    for i in list(sc_ent[1]):
                        posion_idxs.append(i)
                    sc_new_feats.remove(sc_ent)
            sc_new_feats.append(['Donor', tuple(set(donor_idxs))])
            sc_new_feats.append(['PosIonizable', tuple(set(posion_idxs))])
            for sc_new_feat in sc_new_feats:
                sc_pharm_coords, sc_ent = obtain_sc_pharm_ent([sc_new_feat], sc_conf, dummy_pos_list, anchor_1, anchor_2)
                if sc_pharm_coords is not None and sc_ent is not None:
                    coords, fake_lig = create_sc_linker_pharm(dummy_pos_list, sc_pharm_coords, atom_pos_list, antisymbol, pharms, sc_ent, fake_lig, coords, dummy_count)
            ...
            sc_copy_feats = copy.deepcopy(sc_new_feats)
            for sc_ent in sc_copy_feats:
                if sc_ent[0] == 'NegIonizable':
                    for i in list(sc_ent[1]):
                        negion_idxs.append(i)
                    sc_new_feats.remove(sc_ent)
                elif sc_ent[0] == 'Acceptor':
                    sc_new_feats.remove(sc_ent)
            acceptor_idxs = negion_idxs
            sc_new_feats.append(['Acceptor', tuple(set(acceptor_idxs))])
            sc_new_feats.append(['NegIonizable', tuple(set(negion_idxs))])
            for sc_new_feat in sc_new_feats:
                sc_pharm_coords, sc_ent = obtain_sc_pharm_ent([sc_new_feat], sc_conf, dummy_pos_list, anchor_1, anchor_2)
                if sc_pharm_coords is not None and sc_ent is not None:
                    coords, fake_lig = create_sc_linker_pharm(dummy_pos_list, sc_pharm_coords, atom_pos_list, antisymbol, pharms, sc_ent, fake_lig, coords, dummy_count)
            ...
            sc_copy_feats = copy.deepcopy(sc_new_feats)
            for sc_ent in sc_copy_feats:
                if sc_ent[0] == 'PosIonizable':
                    sc_new_feats.remove(sc_ent)
            sc_pharm_coords, sc_ent = obtain_sc_pharm_ent(sc_new_feats, sc_conf, dummy_pos_list, anchor_1, anchor_2)
            if sc_pharm_coords is not None and sc_ent is not None:
                coords, fake_lig = create_sc_linker_pharm(dummy_pos_list, sc_pharm_coords, atom_pos_list, antisymbol, pharms, sc_ent, fake_lig, coords, dummy_count)
        
        elif res == 'LEU' or res == 'ILE' or res == 'VAL' or res == 'PRO':
            sc_new_feats = []
            hydrophobe_idxs = []
            for atom in sc_mol.GetAtoms():
                hydrophobe_idxs.append(atom.GetIdx())
            sc_new_feats.append(['LumpedHydrophobe', tuple(set(hydrophobe_idxs))])
            sc_pharm_coords, sc_ent = obtain_sc_pharm_ent(sc_new_feats, sc_conf, dummy_pos_list, anchor_1, anchor_2)
            if sc_pharm_coords is not None and sc_ent is not None:
                coords, fake_lig = create_sc_linker_pharm(dummy_pos_list, sc_pharm_coords, atom_pos_list, antisymbol, pharms, sc_ent, fake_lig, coords, dummy_count)
