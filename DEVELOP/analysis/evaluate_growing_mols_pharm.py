...
# Determine R_groups of generated molecules
R_groups = Parallel(n_jobs=n_cores)(delayed(frag_utils.get_R_group)(Chem.MolFromSmiles(m[2]), Chem.MolFromSmiles(m[3]), m[1])                                    for m in results)
...
for i, R_group in enumerate(R_groups):
    if R_group == "None":
        continue
    try:
        R_group_canon = Chem.MolFromSmiles(re.sub('[0-9]+\*', '*', R_group))
        Chem.rdmolops.RemoveStereochemistry(R_group_canon)
        R_groups[i] = MolStandardize.canonicalize_tautomer_smiles(Chem.MolToSmiles(R_group_canon))
    except:
        continue

# Update results
for i in range(len(results)):
    results[i].append(R_groups[i])
...
# Update results according to no sulfur linking to one sulfur or two nitrogens 
...
# Update results according to the pharmacophore of R_groups
with open(data_path + '/molecules_' + version + '_out.json', 'r') as json_file:
    json_dict = json.load(json_file)[0]
R_group_pharm = json_dict['abs_dist'][-3:]
print('R_group pharmacophore: ', R_group_pharm)

results_copy = copy.deepcopy(results)
for result in results_copy:
    R_group_mol = Chem.MolFromSmiles(result[-1])
    pharms = ['Donor', 'Acceptor', 'Aromatic']
    dic = {'Donor': 0, 'Acceptor': 0, 'Aromatic': 0}
    dic_list = []
    if R_group_mol is not None:
        feats = factory.GetFeaturesForMol(R_group_mol)
        for feat in feats:
