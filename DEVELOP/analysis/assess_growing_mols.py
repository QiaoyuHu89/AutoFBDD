...
def assess_mols_RMSD(dataset, gen_smi, frag_sdf, train_set_path, save_path, output_name, \
                num_cores, verbose, restrict, pains_smarts_loc):
    os.system('python evaluate_growing_mols_edit.py ' + dataset + ' ' + gen_smi + ' ' + frag_sdf + ' ' + \
            train_set_path + ' ' + save_path + ' ' + output_name + ' ' + num_cores + ' ' + verbose + ' ' + \
            restrict + ' ' + pains_smarts_loc + ' >> ' + output_name + '_RMSD_log.txt')
    
# evaluate the generated molecules using 2D and 3D analysis according to R group pharmacophore
def assess_mols_pharm(dataset, gen_smi, frag_sdf, train_set_path, save_path, output_name, \
                num_cores, verbose, restrict, pains_smarts_loc, data_path):
    os.system('python evaluate_growing_mols_pharm.py ' + dataset + ' ' + gen_smi + ' ' + frag_sdf + ' ' + \
            train_set_path + ' ' + save_path + ' ' + output_name + ' ' + num_cores + ' ' + verbose + ' ' + \
            restrict + ' ' + pains_smarts_loc + ' ' + data_path + ' >> ' + output_name + '_pharm_log.txt')
...
def main(data_path, dataset, gen_smi_folder, train_set_path, save_path, num_cores, verbose, restrict, pains_smarts_loc, iter_idx):
    ...
    for smi in smi_list:
        ...
        delete_du(frag_sdf)
        assess_mols_RMSD(dataset, gen_smi, frag_sdf, train_set_path, save_path, name, \
                str(num_cores), verbose, restrict, pains_smarts_loc)
        ...
        assess_mols_pharm(dataset, gen_smi, frag_sdf, train_set_path, save_path, name, \
                str(num_cores), verbose, restrict, pains_smarts_loc, data_path)
...
if __name__=="__main__":
    # usage: python assess_growing_mols.py -d data_path -a ZINC -f generated_smi -t ../data/linker_design/data_zinc_train.txt 
    ...
    start = time.time()
    main(data_path, dataset, gen_smi_folder, train_set_path, save_path, num_cores, verbose, restrict, pains_smarts_loc, iter_idx)
    end = time.time()
    print('='*100)
    print('Running time: %s Seconds' % (end-start))
    print('Evaluation generated molecules done!')