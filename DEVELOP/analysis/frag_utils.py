...
def get_R_group(full_mol, clean_frag, starting_point):
    matches = list(full_mol.GetSubstructMatches(clean_frag))
    R_group_len = full_mol.GetNumHeavyAtoms() - clean_frag.GetNumHeavyAtoms()
    ...
    if R_group_len != 0:
        ...
        if len(matches)>0:
            for match in matches:
                mol_rw = Chem.RWMol(full_mol)
                R_group_atoms = list(set(list(range(full_mol.GetNumHeavyAtoms()))).difference(match))
                frag_atoms = list(set(list(range(full_mol.GetNumHeavyAtoms()))).difference(match))
                for idx_to_delete in sorted(match, reverse=True):
                    mol_rw.RemoveAtom(idx_to_delete)
                R_group = Chem.Mol(mol_rw)
                if R_group.GetNumHeavyAtoms() == R_group_len:
                    return Chem.MolToSmiles(Chem.MolFromSmiles(Chem.MolToSmiles(R_group)))
