...
def main(name, pdbfile, centerfile, eval_folder, num_pose, iter_idx):
    ...
    surround_mol = Chem.MolFromMol2File('surround_mol_' + str(iter_idx) + '/' + os.path.basename(name) + 
                                              '_surround.mol2')
    ...
    if os.path.exists(name + '/final/aligned_mols.sdf') and os.path.getsize(name + '/final/aligned_mols.sdf') != 0:
        ...
        for i in range(len(aligned_mols)):
            min_dist = cal_sur_dist(aligned_mols[i], surround_mol)
            if min_dist >= 1.5:
...
if __name__=="__main__":
    # Usage: python growing_postprocess.py -i target.pdb -c center.txt -e eval_folder -n 20 -p 3 -t iter_idx
    # Get the arguments
    ...
    print('Bricks growing postprocess done!')