...
def Clustering(pdbfile, bricklist):
    ...
    with open(bricklist, 'r') as fr:
        lines = fr.readlines()
    if len(lines) <= 10:
        os.system("perl cluster.pl %s 1 %s  1 3 1 3 cluster_out" % (pdbname, bricklist))
    elif len(lines) <= 100:
        os.system("perl cluster.pl %s 1 %s  1 3 2 2 cluster_out" % (pdbname, bricklist))
    elif len(lines) <= 1000:
        ...
        os.system("perl cluster.pl %s 1 %s  3 2 10 0.5 cluster_out" % (pdbname, bricklist))
    elif len(lines) > 50000:
        os.system("perl cluster.pl %s 1 %s  3 1 20 0.2 cluster_out" % (pdbname, bricklist))
